import type { chatbot_attempt, chatbot_attempt_message } from '@prisma/client';
import { removeBigInt } from '../helpers'
...
// MessageType is 1 for a user and 2 for a chatbot
async function storeMessage(attemptId: number, messageText: string, messageType: number) {
  let newMessage: chatbot_attempt_message = {
    attempt_id: BigInt(attemptId),
    created_at: new Date(),
    text: messageText,
    message_type: BigInt(messageType)
  }

  const result = await fetch('/api/chatbot_messages/', {
    method: 'POST',
    body: JSON.stringify(removeBigInt(newMessage))
  });
